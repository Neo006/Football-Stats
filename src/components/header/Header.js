...
function Header() {
  ...
  const menuToggle = () => {
    if (isNavOpen) {
      navElement.current.classList.add('hidden');
      burgerButton.current.classList.remove('close');
      setIsNavOpen(false);
    } else {
      navElement.current.classList.remove('hidden');
      burgerButton.current.classList.add('close');
      setIsNavOpen(true);
    }
  };
  ...
  return (
    <header className="relative z-50 w-full h-24">
      <div className="container flex items-center justify-center h-full max-w-6xl px-8 mx-auto sm:justify-between xl:px-0">
        ...
        <nav
          id="nav"
          className="absolute top-0 left-0 z-50 flex flex-col items-center justify-between w-full h-64 pt-5 mt-24 text-sm text-gray-800 bg-white border-t border-gray-200 md:w-auto md:flex-row md:h-24 lg:text-base md:bg-transparent md:mt-0 md:border-none md:py-0 md:flex md:relative hidden"
          ref={navElement}
        >
          <a
            href="#"
          ...
          <a href="#testimonials" className="font-bold duration-100 transition-color hover:text-indigo-600">
            Testimonials
          </a>
          <div className="flex flex-col block w-full font-medium border-t border-gray-200 md:hidden">
            <a href="#_" className="w-full py-2 font-bold text-center text-pink-500">
              Login
            </a>
            <a
              href="#_"
